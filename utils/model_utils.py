...
#  permet d'initialiser les poids d'une couche du réseau
...
# permet d'initialiser un réseau de neurones avec une architecture donnée
...
# permet de faire une copie profonde du réseau pour sauvegarder les meilleurs poids lors de l'entraînement
...
# la dérivée du sigmoid peut être calculée à partir de sa valeur d'activation
def sigmoid_derivative_from_activation(a):
    return a * (1.0 - a)

# la fonction softmax pour la couche de sortie du réseau, qui convertit les valeurs de sortie en probabilités
def softmax(values):
...
# permet de calculer les sorties d'une couche du réseau à partir des entrées, des poids, des biais et de la fonction d'activation
...
# permet de calculer les métriques de classification (accuracy, precision, recall, f1)
...
# permet de faire une passe avant pour un échantillon donné, en calculant les sorties de chaque couche du réseau et en stockant les activations dans un cache pour la rétropropagation
...
# permet de calculer la loss d'entropie croisée entre les étiquettes vraies et les probabilités prédites par le réseau,
# en ajoutant une petite valeur epsilon pour éviter les problèmes de log(0)
...
# permet de prédire la classe (0 ou 1) à partir des probabilités prédites par le réseau, en choisissant la classe avec la probabilité la plus élevée
def predict_class(probabilities):
    if probabilities[1] > probabilities[0]:
        return 1
    return 0

# permet d'évaluer les performances du réseau sur un dataset donné, en calculant la loss moyenne et l'accuracy sur l'ensemble du dataset
...
# permet de calculer les gradients des poids et des biais du réseau à partir des activations stockées dans le cache et des étiquettes vraies, en utilisant la rétropropagation
...
# permet de mettre à jour les poids et les biais du réseau en soustrayant les gradients multipliés par le taux d'apprentissage, en effectuant une descente de gradient stochastique
...
# permet d'entraîner le réseau de neurones sur les données d'entraînement,
# en évaluant les performances sur les données de validation à chaque époque 
# et en utilisant l'arrêt précoce pour éviter le surapprentissage
def train_model(
    network,
    X_train,
    y_train,
...
# permet de sauvegarder le modèle entraîné (poids et biais du réseau) ainsi que les statistiques de normalisation (moyennes et écarts-types) dans un fichier JSON
